#!/usr/bin/env python3
"""
ディズニーランド待ち時間予測 - コマンドラインインターフェース
...
# 手動で設定する場合: MANUAL_CLOSED_ATTRACTIONS_LAND に追加
MANUAL_CLOSED_ATTRACTIONS_LAND = [
...
try:
    from fetch_closed_attractions import get_closed_attractions_land, normalize_attraction_name
...
def get_closed_attractions(target_date=None, attraction_list=None):
    ...
    if AUTO_FETCH_CLOSED:
        try:
            auto_closed = get_closed_attractions_land(target_date)
            if auto_closed:
    ...
    if MANUAL_CLOSED_ATTRACTIONS_LAND:
        print(f"📋 手動リストから休止情報を使用: {len(MANUAL_CLOSED_ATTRACTIONS_LAND)}件")
        return MANUAL_CLOSED_ATTRACTIONS_LAND
    
    return []

# 予測システムをインポート（最新版を優先）
try:
    from disneyland_wait_time_predictor_v3 import DisneyLandWaitTimePredictorV3 as DisneySeaWaitTimePredictor
    WEATHER_INTEGRATED = True
    MODEL_VERSION = "v3"
    MODEL_PATH = "models_land_v3/wait_time_models.joblib"
except ImportError:
    try:
        from disneyland_wait_time_predictor_v2 import DisneySeaWaitTimePredictorV2 as DisneySeaWaitTimePredictor
        WEATHER_INTEGRATED = True
        MODEL_VERSION = "v2"
        MODEL_PATH = "models_v2/wait_time_models.joblib"
    except ImportError:
        from disneyland_wait_time_predictor import DisneySeaWaitTimePredictor
...
# モバイルPDF生成をインポート
try:
    from generate_mobile_pdf_land import create_timetable_page, create_summary_page, round_up_to_10
    HAS_MOBILE_PDF = True
except ImportError:
...
def generate_prediction_report(predictions, date, temperature, is_rainy, output_dir="predictions"):
    ...
    report = []
    report.append("=" * 70)
    report.append(f"🏰 ディズニーランド 待ち時間予測レポート")
    report.append("=" * 70)
...
def generate_hourly_chart(predictions, date, output_dir="predictions"):
    ...
    day_name = get_day_of_week_ja(date)
    ax.set_title(f'ディズニーランド 人気アトラクション時間推移予測\n{date} ({day_name}曜日)', fontsize=14)
...
def main():
    parser = argparse.ArgumentParser(
        description='ディズニーランド待ち時間予測システム',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
使用例:
  # 明日の予測
  python disneyland_predict_cli.py --date 2025-07-20

  # 特定日の予測（気温・天気指定）
  python disneyland_predict_cli.py --date 2025-08-15 --temperature 32 --rainy

  # 複数日の予測
  python disneyland_predict_cli.py --date 2025-07-20 --days 7

  # グラフなしで予測
  python disneyland_predict_cli.py --date 2025-07-20 --no-charts
        """
    )
