...
def parse_yosocal_table_final(html_content):
    """最終版：平均行の検出も含めてHTMLテーブルを解析"""
...
def analyze_results(df):
    ...
    print(f"\n🎢 yosocal.com アトラクション待ち時間データ分析（最終版）")
    print("=" * 80)
    ...
    if not time_915_data.empty:
        ...
        if not operating_data.empty:
            ...
            for idx, (_, row) in enumerate(operating_sorted.head(10).iterrows()):
                print(f"  {row['attraction']:<20}: {row['wait_time']:>3}分")

def main():
    """メイン実行関数"""
    print("🏰 yosocal.com HTMLテーブル解析（最終版）")
    print("=" * 80)
    
    # 最終版で解析
    df = parse_yosocal_table_final(SAMPLE_HTML)
    
    if not df.empty:
        # CSV出力
        timestamp = datetime.now().strftime('%Y%m%d_%H%M')
        filename = f"yosocal_final_{timestamp}.csv"
