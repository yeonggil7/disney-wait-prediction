...
year = 2024
output_dir = "disney_monthly_data"  # 月別データ保存ディレクトリ
...
def main():
    """メイン処理関数"""
    print("🏰 Disney Monthly Scraper for 2024")
    ...
    for month in range(1, 13):
        try:
            ...
            print(f"\n⚠️ Interrupted by user. Stopping at month {month}")
            break
        except Exception as e:
    ...
    if all_monthly_data:
        ...
        annual_filename = f"{output_dir}/disney_2024_annual.csv"
        annual_df.to_csv(annual_filename, index=False, encoding='utf-8-sig')
        
        print(f"\n✅ 2024 Disney Data Collection Completed!")
        print(f"📁 Annual file: {annual_filename}")
        print(f"📊 Total records: {total_records:,}")
        print(f"📈 Success rate: {total_success}/{total_success + total_errors}")
        print(f"🎪 Unique items: {annual_df['item'].nunique()}")
        print(f"🏰 Park breakdown:")
        for park_name, count in annual_df['park'].value_counts().items():
