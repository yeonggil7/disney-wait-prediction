...
def create_timetable_page(predictions, date_str, output_file):
    ...
    title = f"🏰 DisneySea 待ち時間予測  {date_str} ({day_name_ja}) {weather_icon}{avg_temp:.0f}°C"
    if special:
        title += f"  {special}"
...
def create_summary_page(predictions, date_str, output_file):
    ...
    header_text = f"🏰 DisneySea Summary"
...
def main():
    parser = argparse.ArgumentParser(
        description='ディズニーシー待ち時間予測 モバイルPDF生成',
