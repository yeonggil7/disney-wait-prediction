# -*- coding: utf-8 -*-
"""
yosocal.com 修正版長期間データ取得システム
2024年1月1日から2025年6月30日までの全期間データ取得（div要素対応）
"""
...
def extract_calendar_dates_fixed(driver):
    """カレンダーから利用可能な日付を抽出（修正版）"""
    try:
...
def click_date_and_extract_data_fixed(driver, date_num, year, month):
    """日付をクリックしてデータを抽出（修正版）"""
    try:
        # div要素の日付をクリック（複数クラスを考慮）
        xpath_patterns = [
            f"//div[@class='CAL'][text()='{date_num}']",
            f"//div[@class='CALSAT'][text()='{date_num}']", 
            f"//div[@class='CALSUN'][text()='{date_num}']",
            f"//div[contains(@class, 'CAL')][text()='{date_num}']",
            f"//div[@onclick][text()='{date_num}']"
        ]
        
        # 1日の場合は '1/1' も試す
        if date_num == 1:
            xpath_patterns.extend([
                f"//div[@class='CALSUN'][text()='1/1']",
                f"//div[contains(@class, 'CAL')][text()='1/1']"
            ])
...
def main():
    """メイン修正版長期間データ取得プロセス"""
    print("🚀 yosocal.com 修正版長期間データ取得システム")
    ...
    output_file = f"yosocal_long_term_data_fixed_{timestamp}.csv"
    progress_file = f"yosocal_progress_fixed_{timestamp}.json"
    ...
    try:
        ...
        for year, month, month_key in tqdm(target_months, desc="月処理"):
            ...
            try:
                ...
                # カレンダー日付抽出
                available_dates = extract_calendar_dates_fixed(driver)
                ...
                for date_num in tqdm(available_dates, desc=f"{year}/{month:02d}", leave=False):
                    try:
                        # メインページに戻って月を再設定
                        ...
                        # 日付クリックとデータ抽出
                        data, status = click_date_and_extract_data_fixed(driver, date_num, year, month)
                        
                        if data:
                            month_data.extend(data)
                            success_count += 1
                            if len(data) >= 100:  # 十分なデータが取得できた場合
                ...
                print(f"✅ {year}年{month}月完了: {len(month_data)}件 (成功日数: {success_count}/{len(available_dates)})")
                
                # 進捗保存
                save_progress(progress_file, list(completed_months), len(all_data))
                
                # 中間保存（5ヶ月ごと）
                if len(completed_months) % 5 == 0 and len(all_data) > 0:
