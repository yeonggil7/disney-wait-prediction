# -*- coding: utf-8 -*-
"""
yosocal.com realtime.htm起点最終版長期間データ取得システム
2024年1月1日から2025年6月30日までの全期間データ取得
詳細進捗表示・エラーハンドリング・レジューム機能完備
"""
...
def setup_driver_final():
    """最終版WebDriverセットアップ"""
    print("🔧 Chrome WebDriver（最終版）をセットアップ中...")
...
def main():
    """メインrealtime.htm起点最終版プロセス"""
    print("🚀 yosocal.com realtime.htm起点最終版長期間データ取得システム")
    print("📅 対象期間: 2024年1月1日 - 2025年6月30日")
    print("⚡ realtime.htm直接アクセス最適化版")
    print("🎯 詳細進捗表示・エラーハンドリング・レジューム機能完備")
    print("=" * 60)
    
    # 出力ファイル設定
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    output_file = f"yosocal_realtime_final_{timestamp}.csv"
    progress_file = f"yosocal_realtime_final_progress_{timestamp}.json"
            ...
            for d in all_data:
                if d['wait_time'] is not None:
                    ...
                    if name not in attractions:
                        attractions[name] = []
                    attractions[name].append(d['wait_time'])
            
            if attractions:
                print(f"\n🎢 人気アトラクション トップ10（平均待ち時間）:")
                attraction_avg = [(name, sum(times)/len(times)) for name, times in attractions.items()]
                attraction_avg.sort(key=lambda x: x[1], reverse=True)
                
                for i, (name, avg_time) in enumerate(attraction_avg[:10], 1):
                    print(f"   {i:2d}. {name}: {avg_time:.1f}分 ({len(attractions[name])}件)")
            
            # ファイルサイズ
            file_size = os.path.getsize(output_file) / (1024 * 1024)
            print(f"   💾 ファイルサイズ: {file_size:.1f}MB")
        
        print(f"\n⚡ realtime.htm起点最終版処理完了！")
